...
class TransactionUtil extends Util
{
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'total_tender' => request()->total_tender,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $total_tender = !empty(request()->total_tender) ? request()->total_tender : $transaction->payment_lines[0]->total_tender;
            $change_due = $total_tender - $transaction->final_total;
            ...
            $output['total_tender_label'] = 'Tender:';
            $output['total_tender_value'] = !empty($total_tender) ? $this->num_f($total_tender, $show_currency, $business_details) : 0;
            $output['change_due_label'] = 'Change Due:';
            $output['change_due_unformatted'] = !empty($total_tender) ? $change_due : 0;
            $output['change_due_value'] = !empty($total_tender) ? $this->num_f($change_due, $show_currency, $business_details) : 0;
