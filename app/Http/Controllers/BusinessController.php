...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $devices = [];
        if(!empty($pos_settings['nmi_security_key'])) {
            $nmiPG = new NMIPaymentGateway;
            $devices = $nmiPG->getNMIRegisteredDevices($pos_settings['nmi_security_key']);
            if(array_key_exists('poiDevices', $devices)) $devices = $devices['poiDevices'];
        }

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'devices'));
    }
