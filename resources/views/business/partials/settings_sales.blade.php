<div class="pos-tab-content">
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <h4>Diadem Payments:</h4>
        </div>
        <div class="col-sm-6">
            <div class="form-group">
                {!! Form::label('nmi_security_key', __('lang_v1.nmi_security_key') . ':') !!}
                {!! Form::text('pos_settings[nmi_security_key]', $pos_settings['nmi_security_key'] ?? '', ['class' => 'form-control', 'id' => 'nmi_security_key']); !!}
            </div>
        </div>
        <div class="col-sm-6">
            <div class="form-group">
                <!-- {!! Form::label('nmi_terminal_id', __('lang_v1.nmi_terminal_id') . ':') !!}
                {!! Form::text('pos_settings[nmi_terminal_id]', $pos_settings['nmi_terminal_id'] ?? '', ['class' => 'form-control', 'id' => 'nmi_terminal_id']); !!} -->
                @if(count($devices) > 0)
                    {!! Form::label('nmi_terminal_id', __('lang_v1.nmi_terminal_id') . ':') !!}
                    <select class="form-control" name="pos_settings[nmi_terminal_id]" id="nmi_terminal_id">    
                        <option selected disabled>Select a terminal</option>
                        @foreach($devices as $device)
                            <option value="{{ $device['deviceId'] }}"
                                {{ isset($pos_settings['nmi_terminal_id']) && $pos_settings['nmi_terminal_id'] === $device['deviceId'] ? 'selected' : '' }}
                            >
                            {{ $device['deviceNickname'] }} - {{ $device['deviceId'] }}
                        </option>
                        @endforeach
                    </select>
                @else
                    {!! Form::label('nmi_terminal_id', __('lang_v1.nmi_terminal_id') . ' (Not configured):') !!}
                    {!! Form::text('pos_settings[nmi_terminal_id]', '',['class' => 'form-control', 'id' => 'nmi_terminal_id', 'disabled']); !!}
                @endif
            </div>
        </div>
